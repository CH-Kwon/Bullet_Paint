...
void GameLevel1Update(float  dt)
{
	...
	if (WEInput::IsTriggered(WE_3))
	{
		...
		if(WEInput::IsTriggered(WE_MOUSE_LEFT))
		{
			for (int i = 0; i < N; ++i)
			{
				...
				{
					if (Clicked)
						++data.set_score1; //PLZ CHANGE VARIABLE NAME
			...
			if (WEInput::IsTriggered(WE_MOUSE_LEFT))
			{
				for (int i = 0; i < N; ++i)
				{
					...
					{
						if (Clicked)
							data.set_score2++; //PLZ CHANGE VARIABLE NAME
		...
		sprintf_s(data.Score, "%d", (data.set_score1) + (data.set_score2)); // DON'T USE MAGIC NUMBER
		sprintf_s(data.Remain,"%d", 5 - (data.set_score1 + data.set_score2)); // DON'T USE MAGIC NUMBER
		sprintf_s(data.Max,   "%d", 5); // DON'T USE MAGIC NUMBER
        
		if (data.set_score1 + data.set_score2 == 5) // DON'T USE MAGIC NUMBER
		...
		for (data.objectcount = 0; data.objectcount < max_object - 1; ++data.objectcount)
		{
			if (obj[data.objectcount].u_color == Ans_tile[data.objectcount].u_color)
				++check;
			if(check == 9) // CHANGE MAGIC NUMBER TO VARIABLE NAME: maxClick
...
void GameLevel1Shutdown(void)
{
}
void GameLevel1Unload(void)
{
